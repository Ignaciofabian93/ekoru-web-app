...
import type { FooterLinkKey, SocialLink } from "./types";
        ...
        <div className="flex flex-col items-center justify-between gap-4 py-8 md:flex-row">
          ...
          <div className="flex items-center gap-2 text-white/80">
            ...
            <Link
              ...
              className="text-sm text-white/80 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-white/80"
            >
              {t("contact.email")}
            </Link>
          </div>
        ...
        <div className="flex flex-col items-center justify-between gap-4 pt-8 md:flex-row">
          ...
          <div className="flex gap-6">
            {LEGAL_LINKS.map(({ key, route }) => (
              <Link
                key={key}
                // Routes live under `/[lang]`, so the locale prefix is what
                // keeps this resolving instead of 404-ing.
                href={`/${language}${route}`}
                className="text-sm text-white/60 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-white/80"
              >
                {t(`legal.${key}`)}
              </Link>
            ))}
